...
let rooms = applyRoomOverrides([...defaultRooms, ...loadOwnerRooms()]);
...
function isOwnerRoom(room) {
  return room.id.startsWith("owner-");
}
...
function sendVisitEmail(event) {
  event.preventDefault();
  const formData = new FormData(visitForm);
  const room = rooms.find((item) => item.id === formData.get("room"));
  if (!room) return;

  const subject = `Khách muốn xem phòng: ${room.title}`;
  const body = [
    `Phòng: ${room.title}`,
    `Khu vực: ${room.ward}, ${room.district}, ${room.province}`,
    `Địa chỉ: ${room.address}`,
    `Giá: ${formatPrice(room.price)} / tháng`,
    "",
    `Khách: ${formData.get("name")}`,
    `Số điện thoại: ${formData.get("phone")}`,
    `Ngày muốn xem: ${formData.get("date")}`,
    `Ghi chú: ${formData.get("note") || "Không có"}`,
  ].join("\n");

  mailStatus.textContent = "Đang mở ứng dụng email. Khách cần bấm Gửi/Send trong email thì bạn mới nhận được.";
  window.location.href = `mailto:${SETTINGS.businessEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}
...
async function saveOwnerRoom(event) {
  ...
  if (photoFile && photoFile.size) {
    room.photo = await fileToDataUrl(photoFile);
  } else {
    room.photo = existingRoom?.photo || "https://images.unsplash.com/photo-1560185007-c5ca9d2c014d?auto=format&fit=crop&w=1200&q=80";
  }

  if (editingId && existingRoom) {
    const index = ownerRooms.findIndex((item) => item.id === editingId);
    ownerRooms[index] = room;
  } else {
    ownerRooms.unshift(room);
  }

  saveOwnerRooms(ownerRooms);
  resetOwnerForm();
...
function editOwnerRoom(roomId) {
  if (!isOwnerLoggedIn()) return;
  const room = loadOwnerRooms().find((item) => item.id === roomId);
  if (!room) return;
...
function toggleRoomAvailability(roomId) {
  ...
  if (isOwnerRoom(room)) {
    const ownerRooms = loadOwnerRooms();
    const ownerRoom = ownerRooms.find((item) => item.id === roomId);
    if (!ownerRoom) return;
    ownerRoom.available = nextAvailable;
    ownerRoom.status = nextAvailable ? "Còn trống" : "Đã cho thuê";
    saveOwnerRooms(ownerRooms);
  } else {
    const overrides = loadRoomOverrides();
    overrides[roomId] = {
...
ownerLoginForm.addEventListener("submit", (event) => {
  ...
  if (phone === OWNER_ACCOUNT.phone && password === OWNER_ACCOUNT.password) {
    setOwnerLoggedIn(true);
    ownerLoginStatus.textContent = "Đăng nhập thành công.";
    ownerLoginForm.reset();
    updateOwnerView();
    return;
  }

  ownerLoginStatus.textContent = "Sai số điện thoại hoặc mật khẩu.";
});
